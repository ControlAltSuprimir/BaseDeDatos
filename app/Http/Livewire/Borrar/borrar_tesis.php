...
class borrar_tesis extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        return view('livewire.borrar.tesis', [
            'products' => $this->items
        ]);
    }
    ...
    public function delete($productId)
    {
        ...
        $product = Tesis::find($productId);
        if ($product) {
            $product->is_valid=0;
            ProyectosTesistas::where('id_tesis','=',$product->id)->update(['is_valid' => 0]);
            TesisInterna::where('id_tesis','=',$product->id)->update(['is_valid' => 0]);
            PersonasTesisTutores::where('id_tesis','=',$product->id)->update(['is_valid' => 0]);
            PersonasTesisComision::where('id_tesis','=',$product->id)->update(['is_valid' => 0]);
            ArticulosTesis::where('id_tesis','=',$product->id)->update(['is_valid' => 0]);
