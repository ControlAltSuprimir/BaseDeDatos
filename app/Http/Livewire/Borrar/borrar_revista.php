...
class borrar_revista extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        return view('livewire.borrar.revista', [
            'products' => $this->items
        ]);
    }
    ...
    public function delete($productId)
    {
        ...
        $product = Revistas::find($productId);
        if ($product) {
            $product->is_valid=0;
            Indexaciones_Revistas::where('id_Revista','=',$product->id)->update(['is_valid' => 0]);

            $product->save();

            return redirect()->to('/revistas');
        }
    }
}
