<x-app-layout>
  ...
  <form action="{{ route('viajes.update', $data['viaje']) }}" method="POST">
    
      <div class="border-b border-gray-200 px-4 py-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8">
          <div class="flex-1 min-w-0">
              <h1 class="text-lg font-medium leading-6 text-gray-900 sm:truncate">
                  Editar Viaje de
                  {{ $data['viaje']->persona->full_name() }}
              </h1>
          </div>
          <div class="mt-4 flex sm:mt-0 sm:ml-4">
            {{--<input hidden name="persona" value="{{$_GET['persona']}}">--}}
                  ...
                  <div class="bg-white py-6 px-4 sm:p-6">
                      ...
                      <div class="mt-6 grid grid-cols-4 gap-6">
                        <div class="col-span-4 sm:col-span-2">
                          ...
                          <input type="text" name="financiamiento" id="last_name" autocomplete="cc-family-name"
                              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                              value="{{$data['viaje']->financiamiento}}">
                      </div>
                          <div class="col-span-4 sm:col-span-2">
                              <label for="first_name" class="block text-sm font-medium text-gray-700">Fecha del Viaje</label>
                              <input type="date" name="fecha" id="first_name" autocomplete="cc-given-name"
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                  value="{{$data['viaje']->fecha}}">
                          </div>
                          ...
                          <div class="col-span-4 sm:col-span-2">
                              ...
                              <input type="text" name="ciudadOrigen" id="last_name" autocomplete="cc-family-name"
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                  value="{{$data['viaje']->ciudadOrigen}}">
                          </div>

                          <div class="col-span-4 sm:col-span-2">
                              <label for="email_address"
                                  class="block text-sm font-medium text-gray-700">País Destino</label>
                              <input type="text" name="paisDestino" id="email_address" autocomplete="email"
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                  value="{{$data['viaje']->paisDestino}}">                          </div>

                          <div class="col-span-4 sm:col-span-2">
                              <label for="expiration_date" class="block text-sm font-medium text-gray-700">Ciudad Destino</label>
                              <input type="text" name="ciudadDestino" id="expiration_date" autocomplete="cc-exp"
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                  value="{{$data['viaje']->ciudadDestino}}">
                          </div>
