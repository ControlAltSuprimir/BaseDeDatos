<div>
    ...
    <form action="{{ route('revistas.update', $perfil) }}" method="post">
            ...
            <div class="bg-white py-6 px-4 sm:p-6">
                ...
                <div class="col-span-4 sm:col-span-1" gap-6>
                    ...
                    <input type="text" name="nombre" id="expiration_date" autocomplete="cc-exp"
                        ...
                        value="{{ $perfil->nombre }}" placeholder="" required>
                </div>
            ...
            <div class="bg-white py-6 px-4 sm:p-6">
                ...
                <div class="col-span-4 sm:col-span-2">
                    <label for="toogleA" class="flex items-center cursor-pointer">
                        ...
                        <div class="relative">
                            <!-- input -->
                            <input id="toogleA" type="checkbox" class="sr-only" name="Fondecyt" value=1 @if ($perfil->Fondecyt == 1) checked
    @else @endif />
                            <!-- line -->
                ...
                <div class="mt-6 grid grid-cols-4 gap-6">
                    <div class="col-span-4 sm:col-span-2">
                        ...
                        <input type="text" name="clasificacion_Fondecyt" id="alias" autocomplete="cc-given-name"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                            value="{{ $perfil->clasificacion_Fondecyt }}">
                    </div>
            ...
            <div class="bg-white py-6 px-4 sm:p-6">
                ...
                @foreach ($orderProducts as $index => $orderProduct)
                    ...
                    @if (isset($orderProduct['id']))
                        <input hidden name="indexaciones[{{ $index }}][4]" value="{{ $orderProduct['id'] }}">
                @endif
                        <div class="mt-6 grid grid-cols-4 gap-6">
                            <div class="col-span-4 sm:col-span-2">
                                ...
                                <select name="indexaciones[{{ $index }}][0]"
                                    ...
                                    class="mt-1 block w-full bg-white border border-gray-300 roundd-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm">
                                    @if (isset($orderProduct['id']))

                                        <option value="{{ $orderProduct['id_Indexacion'] }}" selected="selected">
                                            {{ $orderProduct['nombre'] }}
                                        </option>
                                        @foreach ($allProducts as $product)
                                            @if ($product->id == $orderProduct['id_Indexacion'])
                                        ...
                                        @endforeach
                                    @else
                                        <option value="">--Escoge una Indexación</option>
                                        @foreach ($allProducts as $product)
                                            <option value="{{ $product->id }}">
                                                {{ $product->nombre }}
                                                {{-- (${{ number_format($product->price, 2) }}) --}}
                            ...
                            <div class="col-span-4 sm:col-span-2">
                                ...
                                <label for="last_name" class="block text-sm font-medium text-gray-700">Clasificación
                                    Q</label>
                                <input type="text" name="indexaciones[{{ $index }}][1]"
                                    id="indexaciones[{{ $index }}][product_id][0]" autocomplete="cc-family-name"
                                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                    @if (isset($orderProduct['id']))
                                    value="{{ $orderProduct['clasificacionQ'] }}"
                                @else
                                value=""
                                @endif
                                    >
                            ...
                            <div class="col-span-4 sm:col-span-2">
                                <label for="last_name" class="block text-sm font-medium text-gray-700">Impact
                                    Factor</label>
                                <input type="number" step=".001" name="indexaciones[{{ $index }}][2]"
                                    id="indexaciones[{{ $index }}][product_id][0]" autocomplete="cc-family-name"
                                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                    @if (isset($orderProduct['id']))
                                    value="{{ $orderProduct['impact_factor'] }}"
                                @else
                                value=""
                                @endif
                                    >
                            ...
                            <div class="col-span-4 sm:col-span-2">
                                <label for="last_name" class="block text-sm font-medium text-gray-700">Observaciones (en
                                    caso de ser no Indexada con o sin referato puede agregar aquí lo que desee)</label>
                                <input type="text" name="indexaciones[{{ $index }}][3]"
                                    id="indexaciones[{{ $index }}][product_id][0]" autocomplete="cc-family-name"
                                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                    @if (isset($orderProduct['id']))
                                    value="{{ $orderProduct['observaciones'] }}"
                                @else
                                value=""
                                @endif
                                    >
