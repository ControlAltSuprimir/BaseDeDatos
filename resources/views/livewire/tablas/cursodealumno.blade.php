                            ...
                            @forelse ($products as $product)
                                ...
                                <tr class="bg-white">
                                    <td class="px-12 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {!! $product->curso->full_nameLink() !!} ({{$product->curso->ano}} / {{$product->curso->periodo}})
                                    </td>
            ...
            <form wire:submit.prevent="save" class="w-full">
                ...
                <div class="flex flex-col items-start p-4">
                    <div class="flex items-center w-full border-b pb-4">
                        <div class="text-gray-900 font-medium text-lg">
                            {{ $productId ? 'Editar Participación' : 'Añadir Curso' }}</div>
                ...
                <div class="bg-white py-6 px-4 sm:p-6">
                    <div class="col-span-4 sm:col-span-1" gap-6>
                        ...
                        <select wire:model.defer="product.id_curso"
                            class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm">
                            <option value="">-- Selecciona el Programa --</option>

                            @foreach ($allCursos as $programa)
                                <option value="{{ $programa->id }}">
                                    {{ $programa->codigo }}: {{ $programa->titulo }} ({{ $programa->ano }} / {{ $programa->periodo }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-6 grid grid-cols-4 gap-6">
                        <div class="col-span-4 sm:col-span-2">
                            <label for="first_name" class="block text-sm font-medium text-gray-700">Nota (de 1.00 a 7.00)</label>
                            <input type="text" autocomplete="cc-given-name"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                wire:model.defer="product.nota">
                        </div>

                        <div class="col-span-4 sm:col-span-2">
                            <label for="last_name" class="block text-sm font-medium text-gray-700">% de Asistencia (de 0.00 a 100.00)</label>
                            <input type="text" autocomplete="cc-family-name"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                wire:model.defer="product.asistencia">
                        </div>

                        <div class="col-span-4 sm:col-span-2">
                            <label for="last_name" class="block text-sm font-medium text-gray-700">Comentarios</label>
                            <input type="text" autocomplete="cc-family-name"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm"
                                wire:model.defer="product.comentarios">
                        </div>
